...
const App: React.FC = () => {
  ...
  const importMembers = (file: File) => {
    ...
    reader.onload = async (event) => {
        ...
        // Get indices for all columns from your image
        ...
        // "Nom complet" and "CNI" are mandatory
        if (nameIndex === -1 || cniIndex === -1) {
          return alert('CSV must contain at least "Nom complet" and "CNI" columns.');
        }
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            // Check for mandatory fields and if the CNI is not a duplicate
            if (name && cni && !existingCnis.has(cni.toLowerCase())) {
                const memberData: Omit<Member, '_id'> = {
                    name,
                    cni,
                    // Check if optional columns exist in the CSV before adding them
                ...
                existingCnis.add(cni.toLowerCase()); // Add to set to prevent duplicates within the same file
            }
        }
        ...
        if (membersToCreate.length > 0) {
            ...
            alert("No new members found to import. They may already exist.");
        }
    };
